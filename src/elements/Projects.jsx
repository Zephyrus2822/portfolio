import { useState } from "react";
import { FaGithub, FaExternalLinkAlt } from "react-icons/fa";
import projects from "../data/projectsData";

const Projects = () => {
  const [mousePos, setMousePos] = useState({ x: 0, y: 0 });

  return (
    <section id="projects" className="bg-black text-white px-6 py-16">
      <h2 className="text-4xl md:text-5xl font-bold text-center mb-12 tracking-wide">
        Projects
      </h2>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-6xl mx-auto items-stretch">
        {projects.map((proj, i) => (
          <div
            key={i}
            className="relative group rounded-xl p-[2px] 
                       bg-gradient-to-r from-purple-500 via-pink-500 to-yellow-500 
                       transition-all duration-500 hover:scale-[1.02]"
            onMouseMove={(e) => {
              const rect = e.currentTarget.getBoundingClientRect();
              setMousePos({
                x: e.clientX - rect.left,
                y: e.clientY - rect.top,
              });
            }}
          >
            {/* Glowing Background Following Cursor */}
            <div
              className="pointer-events-none absolute -inset-px opacity-0 group-hover:opacity-100 
                         transition duration-300 rounded-xl"
              style={{
                background: `radial-gradient(400px circle at ${mousePos.x}px ${mousePos.y}px, rgba(255,255,255,0.1), transparent 70%)`,
              }}
            />

            {/* Card Content */}
            <div className="relative bg-neutral-900 rounded-xl p-6 shadow-lg 
                            transition-all duration-500 group-hover:shadow-purple-500/30 group-hover:shadow-2xl
                            flex flex-col h-full">
              {/* Image */}
              <img
                src={proj.logo}
                alt={`${proj.title} preview`}
                className="w-full h-48 object-cover rounded-lg mb-4 
                           transition-all duration-500 group-hover:scale-[1.03] group-hover:shadow-lg"
              />

              {/* Title & Short */}
              <div className="mb-4">
                <h3 className="text-2xl font-semibold group-hover:text-purple-400 transition-colors">
                  {proj.title}
                </h3>
                <p className="text-sm text-gray-400">{proj.short}</p>
              </div>

              {/* Spacer + Bottom Content */}
              <div className="flex flex-col justify-between flex-grow">
                {/* Links */}
                <div className="flex space-x-4 mb-6">
                  <a
                    href={proj.live_link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-white hover:text-purple-400 transition"
                    title="View Project"
                  >
                    <FaExternalLinkAlt /> Live
                  </a>
                  <a
                    href={proj.github_link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-white hover:text-purple-400 transition"
                    title="View Code"
                  >
                    <FaGithub /> Code
                  </a>
                </div>

                {/* Description */}
                <div className="bg-neutral-800 p-4 rounded-lg mb-4 border border-neutral-700 
                                transition-all duration-300 hover:border-purple-400 flex-grow">
                  <h4 className="font-semibold mb-2 text-lg">Description</h4>
                  <p className="text-sm leading-relaxed text-gray-300">
                    {proj.description}
                  </p>
                </div>

                {/* Tech Stack */}
                <div>
                  <h4 className="font-semibold text-lg mb-2">Tech Stack</h4>
                  <div className="flex flex-wrap gap-2">
                    {proj.tech.map((tech, idx) => (
                      <span
                        key={idx}
                        className="bg-white text-black text-sm font-medium px-3 py-1 rounded-full shadow-sm
                                   transition-all duration-300 hover:scale-105 hover:shadow-purple-400/50"
                      >
                        {tech}
                      </span>
                    ))}
                  </div>
                </div>
              </div>
            </div>
          </div>
        ))}
      </div>
    </section>
  );
};

export default Projects;
